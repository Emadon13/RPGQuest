...
/*!
    ...
    Cette classe place tous les widgets sur la GameWindow qu'elle prend en paramètre
    Elle initialise la vue pour un event ou un allié rejoint notre équipe
    Elle est sensiblement la même que DialogFrame sauf qu'elle ajoute un Hero à la fin du dialogue
*/

AddHeroFrame::AddHeroFrame(GameWindow *g) : QObject()
{
    game=g;
    map=game->GetMap();
...
/*!
    \fn AddHeroFrame::AddHeroFrame::getDialog()

    retourne un vecteur qui représente le dialog et qui contient la liste des dialogElements à jouer.

*/

vector<Dialog> AddHeroFrame::getDialog()
{
...
/*!
    \fn AddHeroFrame::AddHeroFrame::UpdateDialog()

    retourne un vecteur qui représente le dialog et qui contient la liste des dialogElements à jouer.

*/

void AddHeroFrame::UpdateDialog()
{
    int WindowWidth(game->GetGame()->getWindowWidth());
    int WindowHeight(game->GetGame()->getWindowHeight());
